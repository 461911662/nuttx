...
ifeq ($(CONFIG_BOSS1_ESP32S3_FLASH_4M),y)
	FLASH_SIZE := 4MB
else ifeq ($(CONFIG_BOSS1_ESP32S3_FLASH_8M),y)
	FLASH_SIZE := 8MB
else ifeq ($(CONFIG_BOSS1_ESP32S3_FLASH_16M),y)
	FLASH_SIZE := 16MB
else ifeq ($(CONFIG_BOSS1_ESP32S3_FLASH_32M),y)
	FLASH_SIZE := 32MB
endif

ifeq ($(CONFIG_BOSS1_ESP32S3_FLASH_MODE_DIO),y)
	FLASH_MODE := dio
else ifeq ($(CONFIG_BOSS1_ESP32S3_FLASH_MODE_DOUT),y)
	FLASH_MODE := dout
else ifeq ($(CONFIG_BOSS1_ESP32S3_FLASH_MODE_QIO),y)
	FLASH_MODE := qio
else ifeq ($(CONFIG_BOSS1_ESP32S3_FLASH_MODE_QOUT),y)
	FLASH_MODE := qout
endif

ifeq ($(CONFIG_BOSS1_ESP32S3_FLASH_FREQ_120M),y)
	FLASH_FREQ := 120m
else ifeq ($(CONFIG_BOSS1_ESP32S3_FLASH_FREQ_80M),y)
	FLASH_FREQ := 80m
else ifeq ($(CONFIG_BOSS1_ESP32S3_FLASH_FREQ_40M),y)
	FLASH_FREQ := 40m
else ifeq ($(CONFIG_BOSS1_ESP32S3_FLASH_FREQ_20M),y)
	FLASH_FREQ := 20m
endif

ifeq ($(CONFIG_BOSS1_ESP32S3_FLASH_DETECT),y)
...
ifdef ESPTOOL_BINDIR
	ifeq ($(CONFIG_BOSS1_ESP32S3_APP_FORMAT_LEGACY),y)
		BL_OFFSET       := 0x0
		PT_OFFSET       := $(CONFIG_BOSS1_ESP32S3_PARTITION_TABLE_OFFSET)
...
ifeq ($(CONFIG_BOSS1_ESP32S3_APP_FORMAT_LEGACY),y)
	APP_OFFSET     := $(CONFIG_BOSS1_ESP32S3_KERNEL_OFFSET)
	APP_IMAGE      := nuttx.bin
	FLASH_APP      := $(APP_OFFSET) $(APP_IMAGE)
else ifeq ($(CONFIG_BOSS1_ESP32S3_APP_FORMAT_MCUBOOT),y)
	ifeq ($(CONFIG_BOSS1_ESP32S3_ESPTOOL_TARGET_PRIMARY),y)
		VERIFIED   := --confirm
		APP_OFFSET := $(CONFIG_BOSS1_ESP32S3_OTA_PRIMARY_SLOT_OFFSET)
	else ifeq ($(CONFIG_BOSS1_ESP32S3_ESPTOOL_TARGET_SECONDARY),y)
		VERIFIED   :=
		APP_OFFSET := $(CONFIG_BOSS1_ESP32S3_OTA_SECONDARY_SLOT_OFFSET)
	endif
	...
	IMGTOOL_SIGN_ARGS  := --pad $(VERIFIED) $(IMGTOOL_ALIGN_ARGS) -v 0 -s auto \
		-H $(CONFIG_BOSS1_ESP32S3_APP_MCUBOOT_HEADER_SIZE) --pad-header \
		-S $(CONFIG_BOSS1_ESP32S3_OTA_SLOT_SIZE)
else
...
ifeq ($(CONFIG_BUILD_PROTECTED),y)
	ESPTOOL_BINS += $(shell printf "%#x\n" $$(( $(CONFIG_BOSS1_ESP32S3_KERNEL_OFFSET) + $(CONFIG_BOSS1_ESP32S3_KERNEL_IMAGE_SIZE) ))) nuttx_user.bin
endif
...
ifeq ($(CONFIG_BUILD_PROTECTED),y)
define PREBUILD
	$(Q) echo "CONFIG_NUTTX_USERSPACE=$(shell printf "%#x\n" $$(( 0x3c000030 + $(CONFIG_BOSS1_ESP32S3_KERNEL_IMAGE_SIZE) )))" > $(1)/.config.tmp
...
define POSTBUILD
	$(call MKIMAGE)
	$(if $(CONFIG_BOSS1_ESP32S3_MERGE_BINS),$(call MERGEBIN))
endef
...
define FLASH
	...
	$(eval ESPTOOL_OPTS := -c esp32s3 -p $(ESPTOOL_PORT) -b $(ESPTOOL_BAUD) $(if $(CONFIG_BOSS1_ESP32S3_ESPTOOLPY_NO_STUB),--no-stub))
