/****************************************************************************
 * arch/xtensa/src/boss1-esp32/esp32_bt_adapter.c
 *
...
#ifdef CONFIG_ESP32_BT_INTERRUPT_SAVE_STATUS
#  define NR_IRQSTATE_FLAGS   CONFIG_ESP32_BT_INTERRUPT_SAVE_STATUS
#else
...
static xt_handler bt_set_isr(int n, xt_handler f, void *arg);
      ...
      if (bit & mask)
        {
          ...
          wlinfo("s Enabled bit %d\n", irq);
          DEBUGVERIFY(esp32_irq_set_iram_isr(irq));
          up_enable_irq(irq);
          wlinfo("e Enabled bit %d\n", irq);
        }
    }
...
int esp32_bt_controller_init(void)
{
  ...
  cfg.controller_task_stack_size = CONFIG_ESP32_BT_TASK_STACK_SIZE;
  cfg.controller_task_prio       = CONFIG_ESP32_BT_TASK_PRIORITY;
  cfg.bt_max_sync_conn           = CONFIG_BTDM_CTRL_BR_EDR_MAX_SYNC_CONN_EFF;
  cfg.magic                      = ESP_BT_CONTROLLER_CONFIG_MAGIC_VAL;

  wlerr("%s  %d: mode:%x\n", __func__, __LINE__, cfg.mode);
