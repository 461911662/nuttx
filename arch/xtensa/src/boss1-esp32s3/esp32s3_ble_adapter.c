...
#ifdef CONFIG_BOSS1_ESP32S3_BLE_INTERRUPT_SAVE_STATUS
#  define NR_IRQSTATE_FLAGS   CONFIG_BOSS1_ESP32S3_BLE_INTERRUPT_SAVE_STATUS
#else
                                   ...
                                   void **ret_handle)
{
  ...
  cpuint = esp32s3_setup_irq(cpu_id, source, 2, BOSS1_ESP32S3_CPUINT_LEVEL);
  if (cpuint < 0)
    {
      kmm_free(handle);
      return ESP_ERR_NOT_FOUND;
 ...
 *   (CONFIG_BOSS1_ESP32S3_WIFI_BT_COEXIST), it calls the
...
static int coex_schm_register_btdm_callback_wrapper(void *callback)
{
#if CONFIG_BOSS1_ESP32S3_WIFI_BT_COEXIST
 ...
 *   coexistence feature is enabled (CONFIG_BOSS1_ESP32S3_WIFI_BT_COEXIST), it
 ...
 *   (CONFIG_BOSS1_ESP32S3_WIFI_BT_COEXIST), it calls the function and returns its
...
int esp32s3_bt_controller_init(void)
{
  ...
  cfg->controller_task_stack_size = CONFIG_BOSS1_ESP32S3_BLE_TASK_STACK_SIZE;
  cfg->controller_task_prio       = CONFIG_BOSS1_ESP32S3_BLE_TASK_PRIORITY;
