...
static bool    g_dma_chan_used[BOSS1_ESP32S3_DMA_CHAN_MAX];
                            ...
                            bool burst_en)
{
  ...
  DEBUGASSERT(periph  < BOSS1_ESP32S3_DMA_PERIPH_NUM);

  DEBUGASSERT(tx_prio <= BOSS1_ESP32S3_DMA_TX_PRIO_MAX);
  DEBUGASSERT(rx_prio <= BOSS1_ESP32S3_DMA_RX_PRIO_MAX);
  ...
  for (chan = 0; chan < BOSS1_ESP32S3_DMA_CHAN_MAX; chan++)
    {
  ...
  if (chan == BOSS1_ESP32S3_DMA_CHAN_MAX)
    {
  ...
  if (periph == BOSS1_ESP32S3_DMA_PERIPH_MEM)
    {
...
{
  DEBUGASSERT(chan  < BOSS1_ESP32S3_DMA_CHAN_MAX);
                           ...
                           uint8_t *pbuf, uint32_t len, bool tx, int chan)
{
  ...
  uint32_t buf_len;
  int alignment = 4;
  int dma_size = BOSS1_ESP32S3_DMA_BUFFER_MAX_SIZE;
  ...
  DEBUGASSERT(len > 0);
  DEBUGASSERT(chan >= 0 && chan  < BOSS1_ESP32S3_DMA_CHAN_MAX);
  ...
  if (!tx && buffer_in_psram)
    {
      ...
      switch (block_size_index)
        {
          case BOSS1_ESP32S3_DMA_EXT_MEMBLK_64B:
            alignment = 64;
            break;

          case BOSS1_ESP32S3_DMA_EXT_MEMBLK_32B:
            alignment = 32;
            break;

          case BOSS1_ESP32S3_DMA_EXT_MEMBLK_16B:
          default:
            alignment = 16;
            break;
        }
    ...
    {
      dma_size = BOSS1_ESP32S3_DMA_BUFLEN_MAX_4B_ALIGNED;
    }
  ...
  for (i = 0; i < num; i++)
    {
      ...
      dmadesc[i].ctrl = BOSS1_ESP32S3_DMA_CTRL_OWN;
      ...
      if (tx)
        {
          dmadesc[i].ctrl |= (data_len << BOSS1_ESP32S3_DMA_CTRL_DATALEN_S);
        }

      dmadesc[i].ctrl |= (buf_len << BOSS1_ESP32S3_DMA_CTRL_BUFLEN_S);
  ...
  /* suc_eof (defined by BOSS1_ESP32S3_DMA_CTRL_EOF) is set by software
   * only in transmit descriptor.
   */

  if (tx)
    {
      dmadesc[i].ctrl |= BOSS1_ESP32S3_DMA_CTRL_EOF;
    }
