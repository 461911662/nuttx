...
static inline bool is_valid_gpio(uint32_t pin)
{
  ...
  return pin <= 21 || (pin >= 26 && pin < BOSS1_ESP32S3_NPINS);
}
...
static int gpio_interrupt(int irq, void *context, void *arg)
{
  ...
  gpio_dispatch(BOSS1_ESP32S3_FIRST_GPIOIRQ, status, (uint32_t *)context);
  ...
  gpio_dispatch(BOSS1_ESP32S3_FIRST_GPIOIRQ + 32, status, (uint32_t *)context);
  return OK;
}
#endif
...
void esp32s3_gpioirqinitialize(void)
{
  ...
  g_gpio_cpuint = esp32s3_setup_irq(cpu, BOSS1_ESP32S3_PERIPH_GPIO_INT_CPU, 1,
                                    BOSS1_ESP32S3_CPUINT_LEVEL);
  DEBUGASSERT(g_gpio_cpuint >= 0);

  /* Attach and enable the interrupt handler */

  DEBUGVERIFY(irq_attach(BOSS1_ESP32S3_IRQ_GPIO_INT_CPU, gpio_interrupt, NULL));
...
void esp32s3_gpioirqenable(int irq, gpio_intrtype_t intrtype)
{
  ...
  DEBUGASSERT(irq >= BOSS1_ESP32S3_FIRST_GPIOIRQ && irq <= BOSS1_ESP32S3_LAST_GPIOIRQ);

  /* Convert the IRQ number to a pin number */

  pin = BOSS1_ESP32S3_IRQ2PIN(irq);

  /* Disable the GPIO interrupt during the configuration. */

  up_disable_irq(BOSS1_ESP32S3_IRQ_GPIO_INT_CPU);
