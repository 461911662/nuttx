...
#ifdef CONFIG_BOSS1_ESP32S3_LCD_DATA_16BIT
#  define BOSS1_ESP32S3_LCD_DATA_WIDTH  2
#  define BOSS1_ESP32S3_LCD_DATA_BPP    16
#else
#  error "Configure LCD data width is not supported"
#endif

#if CONFIG_BOSS1_ESP32S3_DEFAULT_CPU_FREQ_240
#  if (CONFIG_BOSS1_ESP32S3_LCD_CLOCK_MHZ % 3) == 0
    /* Use PLL=240MHz as clock resource */
#    define BOSS1_ESP32S3_LCD_CLK_SEL   2
#    define BOSS1_ESP32S3_LCD_CLK_MHZ   240
#  else
    /* Use PLL=160MHz as clock resource */
#    define BOSS1_ESP32S3_LCD_CLK_SEL   3
#    define BOSS1_ESP32S3_LCD_CLK_MHZ   160
#  endif
#else
  /* Use PLL=160MHz as clock resource */
#  define BOSS1_ESP32S3_LCD_CLK_SEL     3
...
#define BOSS1_ESP32S3_LCD_PINS          20

/* LCD configuration parameters */

#define BOSS1_ESP32S3_LCD_CLK_N         (BOSS1_ESP32S3_LCD_CLK_MHZ / \
                                   CONFIG_BOSS1_ESP32S3_LCD_CLOCK_MHZ)
#define BOSS1_ESP32S3_LCD_CLK_RES       (BOSS1_ESP32S3_LCD_CLK_MHZ % \
                                   CONFIG_BOSS1_ESP32S3_LCD_CLOCK_MHZ)

#define BOSS1_ESP32S3_LCD_VT_HIGHT      (CONFIG_BOSS1_ESP32S3_LCD_VRES + \
                                   CONFIG_BOSS1_ESP32S3_LCD_VBACKPORCH + \
                                   CONFIG_BOSS1_ESP32S3_LCD_VFRONTPORCH + \
                                   CONFIG_BOSS1_ESP32S3_LCD_VPULSEWIDTH - 1)

#define BOSS1_ESP32S3_LCD_HT_WIDTH      (CONFIG_BOSS1_ESP32S3_LCD_HRES + \
                                   CONFIG_BOSS1_ESP32S3_LCD_HBACKPORCH + \
                                   CONFIG_BOSS1_ESP32S3_LCD_HFRONTPORCH + \
                                   CONFIG_BOSS1_ESP32S3_LCD_HPULSEWIDTH - 1)

#define BOSS1_ESP32S3_LCD_VA_HIGHT      (CONFIG_BOSS1_ESP32S3_LCD_VRES - 1)

#define BOSS1_ESP32S3_LCD_HA_WIDTH      (CONFIG_BOSS1_ESP32S3_LCD_HRES - 1)

#define BOSS1_ESP32S3_LCD_VA_FRONT      (CONFIG_BOSS1_ESP32S3_LCD_VBACKPORCH + \
                                   CONFIG_BOSS1_ESP32S3_LCD_VPULSEWIDTH - 1)

#define BOSS1_ESP32S3_LCD_HB_FRONT      (CONFIG_BOSS1_ESP32S3_LCD_HBACKPORCH + \
                                   CONFIG_BOSS1_ESP32S3_LCD_HPULSEWIDTH -1)

#define BOSS1_ESP32S3_LCD_HSYNC_WIDTH   (CONFIG_BOSS1_ESP32S3_LCD_HPULSEWIDTH - 1)

#define BOSS1_ESP32S3_LCD_VSYNC_WIDTH   (CONFIG_BOSS1_ESP32S3_LCD_VPULSEWIDTH - 1)

#define BOSS1_ESP32S3_LCD_COLOR_FMT     (FB_FMT_RGB16_565)
#define BOSS1_ESP32S3_LCD_STRIDE        (CONFIG_BOSS1_ESP32S3_LCD_HRES * \
                                   BOSS1_ESP32S3_LCD_DATA_WIDTH)

/* Display memory buffer and DMA */

#define BOSS1_ESP32S3_LCD_FB_SIZE       (CONFIG_BOSS1_ESP32S3_LCD_HRES * \
                                   CONFIG_BOSS1_ESP32S3_LCD_VRES * \
                                   BOSS1_ESP32S3_LCD_DATA_WIDTH)

#define BOSS1_ESP32S3_LCD_DMADESC_NUM   (BOSS1_ESP32S3_LCD_FB_SIZE / \
                                   BOSS1_ESP32S3_DMA_BUFLEN_MAX + 1)

#define BOSS1_ESP32S3_LCD_LAYERS        CONFIG_BOSS1_ESP32S3_LCD_BUFFER_LAYERS
...
{
  struct pin_config_s pins_config[BOSS1_ESP32S3_LCD_PINS];
};
...
struct esp32s3_layer_s
{
  ...
  struct esp32s3_dmadesc_s dmadesc[BOSS1_ESP32S3_LCD_DMADESC_NUM];
...
struct esp32s3_lcd_s
{
  ...
  struct esp32s3_layer_s layer[BOSS1_ESP32S3_LCD_LAYERS];
...
#ifdef CONFIG_BOSS1_ESP32S3_LCD_REGDEBUG
...
static const struct esp32s3_lcd_config_s g_lcd_config =
{
  .pins_config =
  {
    { CONFIG_BOSS1_ESP32S3_LCD_PCLK_PIN,   LCD_PCLK_IDX       },
    { CONFIG_BOSS1_ESP32S3_LCD_VSYNC_PIN,  LCD_V_SYNC_IDX     },
    { CONFIG_BOSS1_ESP32S3_LCD_HSYNC_PIN,  LCD_H_SYNC_IDX     },
    { CONFIG_BOSS1_ESP32S3_LCD_HE_PIN,     LCD_H_ENABLE_IDX   },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA0_PIN,  LCD_DATA_OUT0_IDX  },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA1_PIN,  LCD_DATA_OUT1_IDX  },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA2_PIN,  LCD_DATA_OUT2_IDX  },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA3_PIN,  LCD_DATA_OUT3_IDX  },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA4_PIN,  LCD_DATA_OUT4_IDX  },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA5_PIN,  LCD_DATA_OUT5_IDX  },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA6_PIN,  LCD_DATA_OUT6_IDX  },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA7_PIN,  LCD_DATA_OUT7_IDX  },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA8_PIN,  LCD_DATA_OUT8_IDX  },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA9_PIN,  LCD_DATA_OUT9_IDX  },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA10_PIN, LCD_DATA_OUT10_IDX },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA11_PIN, LCD_DATA_OUT11_IDX },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA12_PIN, LCD_DATA_OUT12_IDX },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA13_PIN, LCD_DATA_OUT13_IDX },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA14_PIN, LCD_DATA_OUT14_IDX },
    { CONFIG_BOSS1_ESP32S3_LCD_DATA15_PIN, LCD_DATA_OUT15_IDX }
  }
};
...
{
  .fmt      = BOSS1_ESP32S3_LCD_COLOR_FMT,
  .xres     = CONFIG_BOSS1_ESP32S3_LCD_VRES,
  .yres     = CONFIG_BOSS1_ESP32S3_LCD_HRES,
  .nplanes  = 1
};
  ...
  if (vtable && planeno == 0 && pinfo)
    {
      ...
      pinfo->fblen   = BOSS1_ESP32S3_LCD_FB_SIZE;
      pinfo->stride  = BOSS1_ESP32S3_LCD_STRIDE;
      pinfo->bpp     = BOSS1_ESP32S3_LCD_DATA_BPP;
      return OK;
    }
...
#if BOSS1_ESP32S3_LCD_LAYERS > 1
      priv->cur_layer = (priv->cur_layer + 1) % BOSS1_ESP32S3_LCD_LAYERS;
...
static int esp32s3_lcd_dmasetup(void)
{
  ...
  priv->dma_channel = esp32s3_dma_request(BOSS1_ESP32S3_DMA_PERIPH_LCDCAM,
                                          10, 1, true);
  ...
  esp32s3_dma_set_ext_memblk(priv->dma_channel,
                             true,
                             BOSS1_ESP32S3_DMA_EXT_MEMBLK_64B);

  for (int i = 0; i < BOSS1_ESP32S3_LCD_LAYERS; i++)
    {
      struct esp32s3_layer_s *layer = &priv->layer[i];

      layer->framebuffer = memalign(64, BOSS1_ESP32S3_LCD_FB_SIZE);
      DEBUGASSERT(layer->framebuffer != NULL);
      memset(layer->framebuffer, 0, BOSS1_ESP32S3_LCD_FB_SIZE);

      esp32s3_dma_setup(layer->dmadesc,
                        BOSS1_ESP32S3_LCD_DMADESC_NUM,
                        layer->framebuffer,
                        BOSS1_ESP32S3_LCD_FB_SIZE,
...
static void esp32s3_lcd_gpio_config(void)
{
  ...
  for (int i = 0; i < BOSS1_ESP32S3_LCD_PINS; i++)
    {
...
static void esp32s3_lcd_enableclk(void)
{
...
#if BOSS1_ESP32S3_LCD_CLK_RES != 0
    uint32_t divisor = max_common_divisor(BOSS1_ESP32S3_LCD_CLK_RES,
                                          CONFIG_BOSS1_ESP32S3_LCD_CLOCK_MHZ);
    clk_b = BOSS1_ESP32S3_LCD_CLK_RES / divisor;
    clk_a = CONFIG_BOSS1_ESP32S3_LCD_CLOCK_MHZ / divisor;
  ...
  lcdinfo("PCLK=%d/(%d + %d/%d)\n", BOSS1_ESP32S3_LCD_CLK_MHZ,
          BOSS1_ESP32S3_LCD_CLK_N, clk_b, clk_a);
  ...
  regval = (1 << LCD_CAM_LCD_CLKCNT_N_S) |
           ...
           (BOSS1_ESP32S3_LCD_CLK_SEL << LCD_CAM_LCD_CLK_SEL_S) |
           (BOSS1_ESP32S3_LCD_CLK_N << LCD_CAM_LCD_CLKM_DIV_NUM_S) |
...
static int esp32s3_lcd_config(void)
{
  ...
  regval |= LCD_CAM_LCD_RGB_MODE_EN_M |
            (BOSS1_ESP32S3_LCD_VT_HIGHT << LCD_CAM_LCD_VT_HEIGHT_S) |
            (BOSS1_ESP32S3_LCD_VA_HIGHT << LCD_CAM_LCD_VA_HEIGHT_S) |
            (BOSS1_ESP32S3_LCD_HB_FRONT << LCD_CAM_LCD_HB_FRONT_S);
  esp32s3_lcd_putreg(LCD_CAM_LCD_CTRL_REG, regval);

  regval = (BOSS1_ESP32S3_LCD_HT_WIDTH << LCD_CAM_LCD_HT_WIDTH_S) |
           (BOSS1_ESP32S3_LCD_HA_WIDTH << LCD_CAM_LCD_HA_WIDTH_S) |
           (BOSS1_ESP32S3_LCD_VA_FRONT << LCD_CAM_LCD_VB_FRONT_S);
  esp32s3_lcd_putreg(LCD_CAM_LCD_CTRL1_REG, regval);

  regval = (BOSS1_ESP32S3_LCD_HSYNC_WIDTH << LCD_CAM_LCD_HSYNC_WIDTH_S) |
           (BOSS1_ESP32S3_LCD_VSYNC_WIDTH << LCD_CAM_LCD_VSYNC_WIDTH_S) |
  ...
  priv->cpuint = esp32s3_setup_irq(priv->cpu,
                                   BOSS1_ESP32S3_PERIPH_LCD_CAM,
                                   BOSS1_ESP32S3_INT_PRIO_DEF,
                                   BOSS1_ESP32S3_CPUINT_LEVEL);
  DEBUGASSERT(priv->cpuint >= 0);

  DEBUGASSERT(irq_attach(BOSS1_ESP32S3_IRQ_LCD_CAM, lcd_interrupt, priv) == 0);

  spin_unlock_irqrestore(&priv->lock, flags);

  up_enable_irq(BOSS1_ESP32S3_IRQ_LCD_CAM);
