...
#define WIFI_CONNECT_TIMEOUT  CONFIG_BOSS1_ESP32S3_WIFI_CONNECT_TIMEOUT
...
#if defined(CONFIG_BOSS1_ESP32S3_POWER_SAVE_MIN_MODEM)
#  define DEFAULT_PS_MODE WIFI_PS_MIN_MODEM
#elif defined(CONFIG_BOSS1_ESP32S3_POWER_SAVE_MAX_MODEM)
#  define DEFAULT_PS_MODE WIFI_PS_MAX_MODEM
#elif defined(CONFIG_BOSS1_ESP32S3_POWER_SAVE_NONE)
...
#endif /* CONFIG_BOSS1_ESP32S3_WIFI_BT_COEXIST */
...
static void esp_set_isr(int32_t n, void *f, void *arg)
{
  ...
  ret = irq_attach(BOSS1_ESP32S3_IRQ_MAC, esp_int_adpt_cb, adapter);
  if (ret)
    {
  ...
  ret = irq_attach(BOSS1_ESP32S3_IRQ_PWR, esp_int_adpt_cb, adapter);
  if (ret)
    {
      wlerr("Failed to attach IRQ %d\n", irq);
      PANIC();
...
static void esp32s3_ints_on(uint32_t mask)
{
  ...
  up_enable_irq(BOSS1_ESP32S3_IRQ_MAC);
  up_enable_irq(BOSS1_ESP32S3_IRQ_PWR);
}
...
static void esp32s3_ints_off(uint32_t mask)
{
  ...
  up_disable_irq(BOSS1_ESP32S3_IRQ_MAC);
}
...
static void wifi_apb80m_request(void)
{
#ifdef CONFIG_BOSS1_ESP32S3_AUTO_SLEEP
...
#ifdef CONFIG_BOSS1_ESP32S3_WIFI_TX_AMPDU
  wifi_cfg.ampdu_tx_enable = 1;
#else
  wifi_cfg.ampdu_tx_enable = 0;
#endif

#ifdef CONFIG_BOSS1_ESP32S3_WIFI_RX_AMPDU
  wifi_cfg.ampdu_rx_enable = 1;
#else
  wifi_cfg.ampdu_rx_enable = 0;
#endif

#ifdef CONFIG_BOSS1_ESP32S3_WIFI_STA_DISCONNECT_PM
  wifi_cfg.sta_disconnected_pm = true;
#else
  wifi_cfg.sta_disconnected_pm = false;
#endif

  wifi_cfg.rx_ba_win          = CONFIG_BOSS1_ESP32S3_WIFI_RXBA_AMPDU_WZ;
  wifi_cfg.static_rx_buf_num  = CONFIG_BOSS1_ESP32S3_WIFI_STATIC_RXBUF_NUM;
  wifi_cfg.dynamic_rx_buf_num = CONFIG_BOSS1_ESP32S3_WIFI_DYNAMIC_RXBUF_NUM;
  wifi_cfg.dynamic_tx_buf_num = CONFIG_BOSS1_ESP32S3_WIFI_DYNAMIC_TXBUF_NUM;
