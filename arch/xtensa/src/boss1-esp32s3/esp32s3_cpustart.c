...
static inline void xtensa_attach_fromcpu0_interrupt(void)
{
  ...
  cpuint = esp32s3_setup_irq(1, BOSS1_ESP32S3_PERIPH_INT_FROM_CPU0, 1,
                             BOSS1_ESP32S3_CPUINT_LEVEL);
  DEBUGASSERT(cpuint >= 0);

  /* Attach the inter-CPU interrupt. */

  irq_attach(BOSS1_ESP32S3_IRQ_INT_FROM_CPU0, (xcpt_t)esp32s3_fromcpu0_interrupt,
             NULL);

  /* Enable the inter 0 CPU interrupts. */

  up_enable_irq(BOSS1_ESP32S3_IRQ_INT_FROM_CPU0);
}
