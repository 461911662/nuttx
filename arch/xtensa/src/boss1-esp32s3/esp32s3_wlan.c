...
#define WLAN_PKTBUF_NUM           (CONFIG_BOSS1_ESP32S3_WLAN_PKTBUF_NUM)
...
static struct wlan_priv_s g_wlan_priv[BOSS1_ESP32S3_WLAN_DEVS];
...
{
  struct wlan_priv_s *priv = &g_wlan_priv[BOSS1_ESP32S3_WLAN_STA_DEVNO];
...
{
  struct wlan_priv_s *priv = &g_wlan_priv[BOSS1_ESP32S3_WLAN_SOFTAP_DEVNO];
...
int esp32s3_wlan_sta_initialize(void)
{
  ...
  ret = esp32s3_net_initialize(BOSS1_ESP32S3_WLAN_STA_DEVNO, eth_mac, &g_sta_ops);
  if (ret < 0)
    {
...
int esp32s3_wlan_softap_initialize(void)
{
  ...
  ret = esp32s3_net_initialize(BOSS1_ESP32S3_WLAN_SOFTAP_DEVNO, eth_mac,
                             &g_softap_ops);
  if (ret < 0)
    {
      nerr("ERROR: Failed to initialize net\n");
