...
#ifdef CONFIG_BOSS1_ESP32S3_TOUCH_IRQ
...
static int touch_interrupt(int irq, void *context, void *arg)
{
  ...
  touch_last_irq = BOSS1_ESP32S3_TOUCHPAD2IRQ(pad_num);

  touch_lh_intr_disable(touch_pad_isr_types);

  rt_timer_start(irq_timer_handler,
                 CONFIG_BOSS1_ESP32S3_TOUCH_IRQ_INTERVAL_MS * USEC_PER_MSEC,
                 false);
...
static void touch_init(struct touch_config_s *config)
{
...
#ifdef CONFIG_BOSS1_ESP32S3_TOUCH_FILTER
...
#ifdef CONFIG_BOSS1_ESP32S3_TOUCH_DENOISE
  ...
  ret |= irq_attach(BOSS1_ESP32S3_IRQ_RTC_TOUCH_DONE,
                    touch_interrupt,
                    NULL);

  ret |= irq_attach(BOSS1_ESP32S3_IRQ_RTC_TOUCH_ACTIVE,
                    touch_interrupt,
                    NULL);

  ret |= irq_attach(BOSS1_ESP32S3_IRQ_RTC_TOUCH_INACTIVE,
                    touch_interrupt,
                    NULL);

  ret |= irq_attach(BOSS1_ESP32S3_IRQ_RTC_TOUCH_SCAN_DONE,
                    touch_interrupt,
                    NULL);

  ret |= irq_attach(BOSS1_ESP32S3_IRQ_RTC_TOUCH_TIMEOUT,
                    touch_interrupt,
                    NULL);

  ret |= irq_attach(BOSS1_ESP32S3_IRQ_RTC_TOUCH_APPROACH_LOOP_DONE,
                    touch_interrupt,
                    NULL);
...
{
  DEBUGASSERT(irq >= BOSS1_ESP32S3_FIRST_RTCIOIRQ_TOUCHPAD &&
              irq <= BOSS1_ESP32S3_LAST_RTCIOIRQ_TOUCHPAD);

  int bit = BOSS1_ESP32S3_IRQ2TOUCHPAD(irq);
