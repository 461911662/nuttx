...
#ifdef CONFIG_BOSS1_ESP32S3_FREERUN
...
static int freerun_handler(int irq, void *context, void *arg)
{
  ...
  BOSS1_ESP32S3_TIM_SETALRM(freerun->tch, true); /* Re-enables the alarm */
  BOSS1_ESP32S3_TIM_ACKINT(freerun->tch);        /* Clear the Interrupt */
                               ...
                               uint16_t resolution)
{
  ...
  if (freerun->tch == NULL)
  ...
  else
    {
      ...
      BOSS1_ESP32S3_TIM_STOP(freerun->tch);

      /* Configure clock source */

      BOSS1_ESP32S3_TIM_CLK_SRC(freerun->tch, BOSS1_ESP32S3_TIM_APB_CLK);
      ...
      BOSS1_ESP32S3_TIM_SETPRE(freerun->tch, pre);

      /* Configure TIMER mode */

      BOSS1_ESP32S3_TIM_SETMODE(freerun->tch, BOSS1_ESP32S3_TIM_MODE_UP);

      /* Clear TIMER counter value */

      BOSS1_ESP32S3_TIM_CLEAR(freerun->tch);

      /* Set the maximum timeout */

      BOSS1_ESP32S3_TIM_SETALRVL(freerun->tch, freerun->max_timeout);
      ...
      BOSS1_ESP32S3_TIM_SETARLD(freerun->tch, true);

      /* Enable TIMER alarm */

      BOSS1_ESP32S3_TIM_SETALRM(freerun->tch, true);

      /* Clear Interrupt Bits Status */

      BOSS1_ESP32S3_TIM_ACKINT(freerun->tch);

      /* Register the handler */

        {
          irqstate_t flags = spin_lock_irqsave(&g_lock);
          ret = BOSS1_ESP32S3_TIM_SETISR(freerun->tch, freerun_handler, freerun);
          spin_unlock_irqrestore(&g_lock, flags);
        }

      if (ret == OK)
        {
          BOSS1_ESP32S3_TIM_ENABLEINT(freerun->tch);
        }

#endif
      /* Finally, start the TIMER */

      BOSS1_ESP32S3_TIM_START(freerun->tch);
    }
                            ...
                            struct timespec *ts)
{
  ...
  BOSS1_ESP32S3_TIM_GETCTR(freerun->tch, &counter);
  pending  = BOSS1_ESP32S3_TIM_CHECKINT(freerun->tch);
  BOSS1_ESP32S3_TIM_GETCTR(freerun->tch, &verify);
...
int esp32s3_freerun_uninitialize(struct esp32s3_freerun_s *freerun)
{
  ...
  BOSS1_ESP32S3_TIM_DISABLEINT(freerun->tch);

  /* Detach handler */

  ret = BOSS1_ESP32S3_TIM_SETISR(freerun->tch, NULL, NULL);
...
#endif /* CONFIG_BOSS1_ESP32S3_FREERUN */
