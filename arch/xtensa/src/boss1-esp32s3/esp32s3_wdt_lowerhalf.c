...
static int wdt_lh_ioctl(struct watchdog_lowerhalf_s *lower, int cmd,
                        unsigned long arg);
#ifdef CONFIG_BOSS1_ESP32S3_XTWDT_BACKUP_CLK_ENABLE
...
#ifdef CONFIG_BOSS1_ESP32S3_MWDT0
/* MWDT0 lower-half */
...
#ifdef CONFIG_BOSS1_ESP32S3_MWDT1
/* MWDT1 lower-half */
...
#ifdef CONFIG_BOSS1_ESP32S3_RWDT
/* RWDT lower-half */
...
#ifdef CONFIG_BOSS1_ESP32S3_XTWDT
/* XTWDT lower-half */
...
static int wdt_lh_start(struct watchdog_lowerhalf_s *lower)
{
  ...
  /* If WDT was not started yet */
  ...
  else
    {
      ...
      BOSS1_ESP32S3_WDT_UNLOCK(priv->wdt);
      ...
      if (priv->handler == NULL)
        {
            ...
            if (priv->peripheral == TIMER)
              {
                BOSS1_ESP32S3_WDT_STG_CONF(priv->wdt, BOSS1_ESP32S3_WDT_STAGE0,
                                     BOSS1_ESP32S3_WDT_STAGE_ACTION_RESET_SYSTEM);
              }
            else if (priv->peripheral == RTC)
              {
                BOSS1_ESP32S3_WDT_STG_CONF(priv->wdt, BOSS1_ESP32S3_WDT_STAGE0,
                                     BOSS1_ESP32S3_WDT_STAGE_ACTION_RESET_RTC);
              }
        }
      ...
      /* User handler was already provided */
      ...
      else
        {
          ...
          BOSS1_ESP32S3_WDT_STG_CONF(priv->wdt, BOSS1_ESP32S3_WDT_STAGE0,
                               BOSS1_ESP32S3_WDT_STAGE_ACTION_INT);

          /* Set the lower-half handler and enable interrupt */

          flags = spin_lock_irqsave(&priv->lock);
          BOSS1_ESP32S3_WDT_SETISR(priv->wdt, wdt_handler, priv);
          spin_unlock_irqrestore(&priv->lock, flags);
          BOSS1_ESP32S3_WDT_ENABLEINT(priv->wdt);
        }

      flags = spin_lock_irqsave(&priv->lock);
      priv->lastreset = clock_systime_ticks();
      BOSS1_ESP32S3_WDT_START(priv->wdt);
      spin_unlock_irqrestore(&priv->lock, flags);

      /* Lock it again */

      BOSS1_ESP32S3_WDT_LOCK(priv->wdt);
    }
...
static int wdt_lh_stop(struct watchdog_lowerhalf_s *lower)
{
  ...
  BOSS1_ESP32S3_WDT_STOP(priv->wdt);
  ...
  if (priv->handler != NULL)
    {
      ...
      BOSS1_ESP32S3_WDT_DISABLEINT(priv->wdt);

      flags = spin_lock_irqsave(&priv->lock);
      BOSS1_ESP32S3_WDT_SETISR(priv->wdt, NULL, NULL);
      spin_unlock_irqrestore(&priv->lock, flags);
    }
...
static int wdt_lh_keepalive(struct watchdog_lowerhalf_s *lower)
{
  ...
  flags = spin_lock_irqsave(&priv->lock);
  priv->lastreset = clock_systime_ticks();
  BOSS1_ESP32S3_WDT_FEED(priv->wdt);
  spin_unlock_irqrestore(&priv->lock, flags);
                             ...
                             uint32_t timeout)
{
  ...
  if (priv->peripheral == TIMER)
    {
      ...
      if (timeout == 0 || timeout > MWDT_MAX_TIMEOUT_MS)
        ...
        {
          BOSS1_ESP32S3_WDT_STO(priv->wdt, MWDT_TIMEOUT_MS(timeout),
                          BOSS1_ESP32S3_WDT_STAGE0);
        }
    }
  ...
  /* Watchdog from RTC Module */
    ...
    {
      rtc_cycles = BOSS1_ESP32S3_RWDT_CLK(priv->wdt);
      ...
      if (timeout == 0 || timeout > rtc_ms_max)
      ...
      else
        {
          ...
          BOSS1_ESP32S3_WDT_STO(priv->wdt, timeout, BOSS1_ESP32S3_WDT_STAGE0);
        }
    }
                        ...
                        unsigned long arg)
{
  ...
  switch (cmd)
    {
    case WDIOC_RSTCLK:
      BOSS1_ESP32S3_XTWDT_RST_CLK(priv->wdt);
      break;
...
static int wdt_handler(int irq, void *context, void *arg)
{
  ...
  BOSS1_ESP32S3_WDT_ACKINT(priv->wdt);
...
int esp32s3_wdt_initialize(const char *devpath, enum esp32s3_wdt_inst_e wdt)
{
  ...
  BOSS1_ESP32S3_WDT_UNLOCK(lower->wdt);
  ...
  if (lower->peripheral == TIMER)
    {
      BOSS1_ESP32S3_MWDT_PRE(lower->wdt, MWDT_CLK_PRESCALER_VALUE);
    }
  ...
  if (lower->peripheral == XTAL32K)
    {
      ...
      BOSS1_ESP32S3_XTWDT_PRE(lower->wdt, xtal32k_clk_factor);
    }
#endif

  BOSS1_ESP32S3_WDT_LOCK(lower->wdt);
