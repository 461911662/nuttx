...
#define SWI_IRQ       BOSS1_ESP32S3_IRQ_INT_FROM_CPU2
#define SWI_PERIPH    BOSS1_ESP32S3_PERIPH_INT_FROM_CPU2
...
#ifdef CONFIG_BOSS1_ESP32S3_PHY_INIT_DATA_IN_PARTITION
static const char *phy_partion_label = "phy_init";
#endif

#ifdef CONFIG_BOSS1_ESP32S3_SUPPORT_MULTIPLE_PHY_INIT_DATA
...
#endif /* CONFIG_BOSS1_ESP32S3_WIFI */
  ...
  if (init_data_type != ESP_PHY_INIT_DATA_TYPE_DEFAULT)
    {
      ...
      if (ret != OK)
        {
          kmm_free(init_data_store);
#ifdef CONFIG_BOSS1_ESP32S3_PHY_INIT_DATA_ERROR
          abort();
#else
          return ret;
#endif
...
const esp_phy_init_data_t *esp_phy_get_init_data(void)
{
  ...
  if (memcmp(init_data_store, PHY_INIT_MAGIC, sizeof(phy_init_magic_pre))
      != 0 || memcmp(init_data_store + length - sizeof(phy_init_magic_post),
              PHY_INIT_MAGIC, sizeof(phy_init_magic_post)) != 0)
    {
#ifdef CONFIG_BOSS1_ESP32S3_PHY_DEFAULT_INIT_IF_INVALID
...
#else /* CONFIG_BOSS1_ESP32S3_PHY_DEFAULT_INIT_IF_INVALID */
...
#else /* CONFIG_BOSS1_ESP32S3_PHY_INIT_DATA_IN_PARTITION */
...
int esp_wireless_init(void)
{
  ...
  priv->cpuint = esp32s3_setup_irq(0, SWI_PERIPH, BOSS1_ESP32S3_INT_PRIO_DEF, 0);
  if (priv->cpuint < 0)
    {
