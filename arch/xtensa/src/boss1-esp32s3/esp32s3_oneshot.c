...
#ifdef CONFIG_BOSS1_ESP32S3_ONESHOT
...
static int oneshot_handler(int irq, void *context, void *arg)
{
  ...
  BOSS1_ESP32S3_TIM_DISABLEINT(oneshot->tim);

  /* Detach handler */

  ret = BOSS1_ESP32S3_TIM_SETISR(oneshot->tim, NULL, NULL);

  /* Clear the Interrupt */

  BOSS1_ESP32S3_TIM_ACKINT(oneshot->tim);
                               ...
                               int chan, uint16_t resolution)
{
  ...
  if (oneshot->tim == NULL)
  ...
  else
    {
      ...
      BOSS1_ESP32S3_TIM_STOP(oneshot->tim);

      BOSS1_ESP32S3_TIM_CLK_SRC(oneshot->tim, BOSS1_ESP32S3_TIM_APB_CLK);
      ...
      BOSS1_ESP32S3_TIM_SETPRE(oneshot->tim, pre);
    }
                          ...
                          const struct timespec *ts)
{
  ...
  BOSS1_ESP32S3_TIM_SETMODE(oneshot->tim, BOSS1_ESP32S3_TIM_MODE_UP);

  /* Clear TIMER counter value */

  BOSS1_ESP32S3_TIM_CLEAR(oneshot->tim);

  /* Disable autoreload */

  BOSS1_ESP32S3_TIM_SETARLD(oneshot->tim, false);

  /* Set the timeout */

  BOSS1_ESP32S3_TIM_SETALRVL(oneshot->tim, timeout_us / oneshot->resolution);

  /* Enable TIMER alarm */

  BOSS1_ESP32S3_TIM_SETALRM(oneshot->tim, true);
  ...
  ret = BOSS1_ESP32S3_TIM_SETISR(oneshot->tim, oneshot_handler, oneshot);
  if (ret == OK)
    {
      BOSS1_ESP32S3_TIM_ENABLEINT(oneshot->tim);

      /* Finally, start the TIMER */

      BOSS1_ESP32S3_TIM_START(oneshot->tim);
      ...
      if (ts != NULL)
        {
          ...
          BOSS1_ESP32S3_TIM_GETCTR(oneshot->tim, &counter_value);

          /* Get the current configured timeout */

          BOSS1_ESP32S3_TIM_GETALRVL(oneshot->tim, &alarm_value);
                            ...
                            uint64_t *usec)
{
  ...
  BOSS1_ESP32S3_TIM_GETCTR(oneshot->tim, usec);
...
#endif /* CONFIG_BOSS1_ESP32S3_ONESHOT */
