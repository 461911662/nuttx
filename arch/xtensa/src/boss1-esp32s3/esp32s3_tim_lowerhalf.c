...
#define BOSS1_ESP32S3_TIMER_MAX_USECOND UINT32_MAX
...
#if defined(CONFIG_BOSS1_ESP32S3_TIMER0)
/* TIMER0 lower-half */
...
static int timer_lh_handler(int irq, void *context, void *arg)
{
  ...
  BOSS1_ESP32S3_TIM_ACKINT(priv->tim);        /* Clear the Interrupt */
      ...
      if (next_interval_us > 0)
        {
          ...
          BOSS1_ESP32S3_TIM_SETALRVL(priv->tim, next_interval_us);
        }
    }
  else
    {
  ...
  BOSS1_ESP32S3_TIM_SETALRM(priv->tim, true); /* Re-enables the alarm */
...
static int timer_lh_start(struct timer_lowerhalf_s *lower)
{
  ...
  BOSS1_ESP32S3_TIM_STOP(priv->tim);

  /* Configure clock source */

  BOSS1_ESP32S3_TIM_CLK_SRC(priv->tim, BOSS1_ESP32S3_TIM_APB_CLK);
  ...
  BOSS1_ESP32S3_TIM_SETPRE(priv->tim, pre);

  /* Configure TIMER mode */

  BOSS1_ESP32S3_TIM_SETMODE(priv->tim, BOSS1_ESP32S3_TIM_MODE_UP);

  /* Clear TIMER counter value */

  BOSS1_ESP32S3_TIM_CLEAR(priv->tim);

  /* Enable autoreload */

  BOSS1_ESP32S3_TIM_SETARLD(priv->tim, true);

  /* Enable TIMER alarm */

  BOSS1_ESP32S3_TIM_SETALRM(priv->tim, true);

  /* Clear Interrupt Bits Status */

  BOSS1_ESP32S3_TIM_ACKINT(priv->tim);
  ...
  if (priv->callback != NULL)
    {
      ...
      ret = BOSS1_ESP32S3_TIM_SETISR(priv->tim, timer_lh_handler, priv);
      ...
      BOSS1_ESP32S3_TIM_ENABLEINT(priv->tim);
    }

  /* Finally, start the TIMER */

  BOSS1_ESP32S3_TIM_START(priv->tim);
  priv->started = true;
...
static int timer_lh_stop(struct timer_lowerhalf_s *lower)
{
  ...
  BOSS1_ESP32S3_TIM_DISABLEINT(priv->tim);

  flags = spin_lock_irqsave(&priv->lock);
  ret = BOSS1_ESP32S3_TIM_SETISR(priv->tim, NULL, NULL);
  spin_unlock_irqrestore(&priv->lock, flags);
                              ...
                              struct timer_status_s *status)
{
  ...
  BOSS1_ESP32S3_TIM_GETCTR(priv->tim, &current_counter_value);

  /* Get the current configured timeout */

  BOSS1_ESP32S3_TIM_GETALRVL(priv->tim, &alarm_value);
                               ...
                               uint32_t timeout)
{
  ...
  BOSS1_ESP32S3_TIM_SETALRVL(priv->tim, (uint64_t)timeout);
                               ...
                               uint32_t *max_timeout)
{
  ...
  *max_timeout = BOSS1_ESP32S3_TIMER_MAX_USECOND;
