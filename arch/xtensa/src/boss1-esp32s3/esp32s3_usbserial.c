...
#define BOSS1_ESP32S3_USBCDC_BUFFERSIZE 64
...
static char g_rxbuffer[BOSS1_ESP32S3_USBCDC_BUFFERSIZE];
static char g_txbuffer[BOSS1_ESP32S3_USBCDC_BUFFERSIZE];

static struct esp32s3_priv_s g_usbserial_priv =
{
  .periph = BOSS1_ESP32S3_PERIPH_USB_DEVICE,
  .irq    = BOSS1_ESP32S3_IRQ_USB_DEVICE,
  .cpu    = 0,
  .cpuint = -ENOMEM,
};
...
uart_dev_t g_uart_usbserial =
{
  ...
  .recv      =
    {
      .size    = BOSS1_ESP32S3_USBCDC_BUFFERSIZE,
      .buffer  = g_rxbuffer,
    },
  .xmit      =
    {
