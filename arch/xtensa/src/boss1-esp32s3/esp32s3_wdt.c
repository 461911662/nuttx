...
struct esp32s3_wdt_priv_s g_esp32s3_mwdt0_priv =
{
  ...
  .periph = BOSS1_ESP32S3_PERIPH_TG_WDT_LEVEL,
  .irq    = BOSS1_ESP32S3_IRQ_TG_WDT_LEVEL,
  .cpuint = -ENOMEM,
  .inuse  = false,
  .type   = TIMER,
};
...
struct esp32s3_wdt_priv_s g_esp32s3_mwdt1_priv =
{
  ...
  .periph = BOSS1_ESP32S3_PERIPH_TG1_WDT_LEVEL,
  .irq    = BOSS1_ESP32S3_IRQ_TG1_WDT_LEVEL,
  .cpuint = -ENOMEM,
  .inuse  = false,
  .type   = TIMER,
};
...
struct esp32s3_wdt_priv_s g_esp32s3_rwdt_priv =
{
  ...
  .periph = BOSS1_ESP32S3_PERIPH_RTC_CORE,
  .irq    = BOSS1_ESP32S3_IRQ_RTC_WDT,
  .cpuint = -ENOMEM,
  .inuse  = false,
  .type   = RTC,
};
...
struct esp32s3_wdt_priv_s g_esp32s3_xtwdt_priv =
{
  ...
  .base   = RTC_CNTL_RTC_OPTIONS0_REG,
  .periph = BOSS1_ESP32S3_PERIPH_RTC_CORE,
  .irq    = BOSS1_ESP32S3_IRQ_RTC_XTAL32K_DEAD,
  .cpuint = -ENOMEM,
  .inuse  = false,
  .type   = XTAL32K,
};
  ...
  if (handler == NULL)
    {
      ...
      if (wdt->cpuint >= 0)
        {
#if defined(CONFIG_BOSS1_ESP32S3_RWDT) || defined(CONFIG_BOSS1_ESP32S3_XTWDT)
          if (wdt->irq == BOSS1_ESP32S3_IRQ_RTC_WDT ||
              wdt->irq == BOSS1_ESP32S3_IRQ_RTC_XTAL32K_DEAD)
            {
