...
#if defined(CONFIG_BOSS1_ESP32S3_SDMMC_DMA) && defined(CONFIG_BOSS1_ESP32S3_SPIRAM)
 ...
 *     initialization.  Required if CONFIG_BOSS1_ESP32S3_SDMMC_DMA is enabled.
 *   CONFIG_BOSS1_ESP32S3_DMA2 - Enable ESP32S3 DMA2 support.  Required if
 *     CONFIG_BOSS1_ESP32S3_SDMMC_DMA is enabled
 ...
 *   CONFIG_BOSS1_ESP32S3_SDMMC_DMA - Enable SDIO.  This is a marginally optional.
...
#define BOSS1_ESP32S3_TXFIFO_SIZE       (BOSS1_ESP32S3_TXFIFO_DEPTH | BOSS1_ESP32S3_TXFIFO_WIDTH)
#define BOSS1_ESP32S3_RXFIFO_SIZE       (BOSS1_ESP32S3_RXFIFO_DEPTH | BOSS1_ESP32S3_RXFIFO_WIDTH)

/* Number of DMA Descriptors */

#define BOSS1_ESP32S3_MULTIBLOCK_LIMIT  128
#define NUM_DMA_DESCRIPTORS       (1 + (BOSS1_ESP32S3_MULTIBLOCK_LIMIT * 512 / MCI_DMADES1_MAXTR))

#if (CONFIG_MMCSD_MULTIBLOCK_LIMIT == 0 || \
     CONFIG_MMCSD_MULTIBLOCK_LIMIT > BOSS1_ESP32S3_MULTIBLOCK_LIMIT)
...
#ifdef CONFIG_BOSS1_ESP32S3_SDMMC_DMA
...
/* #define CONFIG_BOSS1_ESP32S3_SDMMC_REGDEBUG */

#ifdef CONFIG_BOSS1_ESP32S3_SDMMC_REGDEBUG
...
static int esp32s3_ciu_sendcmd(uint32_t cmd, uint32_t arg)
{
  ...
  while ((esp32s3_getreg(BOSS1_ESP32S3_SDMMC_CMD) & SDMMC_CMD_STARTCMD) != 0)
    {
      if (watchtime - clock_systime_ticks() > SDCARD_CMDTIMEOUT)
        {
          mcerr("TMO Timed out (%08X)\n",
                esp32s3_getreg(BOSS1_ESP32S3_SDMMC_CMD));
          return 1;
        }
    }
  ...
  esp32s3_putreg(arg, BOSS1_ESP32S3_SDMMC_CMDARG);
  esp32s3_putreg(cmd, BOSS1_ESP32S3_SDMMC_CMD);
...
static void esp32s3_enable_ints(struct esp32s3_dev_s *priv)
{
  ...
  mcinfo("waitmask=%04lx xfrmask=%04lx dmamask=%04lx RINTSTS=%08lx\n",
         ...
         (unsigned long)esp32s3_getreg(BOSS1_ESP32S3_SDMMC_RINTSTS));
#else
  ...
  esp32s3_putreg(regval, BOSS1_ESP32S3_SDMMC_INTMASK);
}
...
static void esp32s3_disable_allints(struct esp32s3_dev_s *priv)
{
  ...
  esp32s3_putreg(SDCARD_INT_CDET, BOSS1_ESP32S3_SDMMC_INTMASK);
  priv->waitmask = 0;
  priv->xfrmask  = 0;
}
                                ...
                                sdio_eventset_t wkupevent)
{
  ...
  esp32s3_putreg(priv->waitmask, BOSS1_ESP32S3_SDMMC_RINTSTS);
...
static int esp32s3_interrupt(int irq, void *context, void *arg)
{
  ...
  while ((enabled = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_MINTSTS)) != 0)
    {
      /* Clear pending status */

      esp32s3_putreg(enabled, BOSS1_ESP32S3_SDMMC_RINTSTS);
      ...
      if ((enabled & SDMMC_INT_CDET) != 0)
        {
          ...
          if ((esp32s3_getreg(BOSS1_ESP32S3_SDMMC_CDETECT) &
              SDMMC_CDETECT_NOTPRESENT(priv->slot)) == 0)
            {
              priv->cdstatus |= SDIO_STATUS_PRESENT;

#ifdef CONFIG_MMCSD_HAVE_WRITEPROTECT
              if ((esp32s3_getreg(BOSS1_ESP32S3_SDMMC_WRTPRT) &
          ...
          if ((pending & SDMMC_INT_TXDR) != 0)
            {
              ...
              for (status = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_STATUS);
                   (status & SDMMC_STATUS_FIFOFULL) == 0 &&
                   priv->remaining > 0;
                   status = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_STATUS))
                {
                  esp32s3_putreg(*priv->buffer, BOSS1_ESP32S3_SDMMC_DATA);
                  priv->buffer++;
                  priv->remaining -= 4;
                }
            }
          else if ((pending & SDMMC_INT_RXDR) != 0)
            {
              ...
              for (status = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_STATUS);
                   ...
                   status = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_STATUS))
                {
                  *priv->buffer = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_DATA);
                  priv->buffer++;
                  priv->remaining -= 4;
                }
            }
  ...
  pending = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_IDSTS);
  if ((pending & priv->dmamask) != 0)
    {
      ...
      esp32s3_putreg(pending, BOSS1_ESP32S3_SDMMC_IDSTS);
...
static void esp32s3_reset(struct sdio_dev_s *dev)
{
  ...
  esp32s3_putreg(SDMMC_CTRL_CNTLRRESET | SDMMC_CTRL_FIFORESET |
                 SDMMC_CTRL_DMARESET, BOSS1_ESP32S3_SDMMC_CTRL);

  while ((esp32s3_getreg(BOSS1_ESP32S3_SDMMC_CTRL) &
  ...
  regval = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_CLOCK);
  ...
  esp32s3_putreg(regval, BOSS1_ESP32S3_SDMMC_CLOCK);

  /* Select clock divider
   * Slot N selects clock divider N.
   */

  regval = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_CLKSRC);
  regval &= ~SDMMC_CLKSRC_MASK(priv->slot);
  regval |= SDMMC_CLKSRC_CLKDIV(priv->slot, priv->slot);
  esp32s3_putreg(regval, BOSS1_ESP32S3_SDMMC_CLKSRC);

#ifdef CONFIG_BOSS1_ESP32S3_SDMMC_DMA
  esp32s3_putreg((uint32_t)&priv->dma_desc[0], BOSS1_ESP32S3_SDMMC_DBADDR);
#endif
...
static void esp32s3_widebus(struct sdio_dev_s *dev, bool wide)
{
  ...
  regval = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_CTYPE);
  ...
  if (wide)
    {
      ...
      configure_pin(CONFIG_BOSS1_ESP32S3_SDMMC_D1, priv->sdio_pins->d1,
                    INPUT | OUTPUT | PULLUP);
      configure_pin(CONFIG_BOSS1_ESP32S3_SDMMC_D2, priv->sdio_pins->d2,
                    INPUT | OUTPUT | PULLUP);
      configure_pin(CONFIG_BOSS1_ESP32S3_SDMMC_D3, priv->sdio_pins->d3,
                    INPUT | OUTPUT | PULLUP);
    }
#endif

  esp32s3_putreg(regval, BOSS1_ESP32S3_SDMMC_CTYPE);
}
  ...
  while (repeat)
    {
      ...
      while (timeout_ms)
        {
          regval = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_RINTSTS);
          if (regval & SDMMC_INT_HLE)
            {
              esp32s3_putreg(SDMMC_INT_HLE, BOSS1_ESP32S3_SDMMC_RINTSTS);
              break;
            }

          if ((esp32s3_getreg(BOSS1_ESP32S3_SDMMC_CMD) & SDMMC_CMD_STARTCMD)
              == 0)
            {
              repeat = false;
              break;
                                   ...
                                   uint32_t card_div)
{
  ...
  regval = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_CLKDIV);
  regval &= ~SDMMC_CLKDIV_MASK(divider);
  regval |= SDMMC_CLKDIV(divider, card_div);
  esp32s3_putreg(regval, BOSS1_ESP32S3_SDMMC_CLKDIV);
...
static void esp32s3_clock(struct sdio_dev_s *dev, enum sdio_clock_e rate)
{
...
#ifndef CONFIG_BOSS1_ESP32S3_SDMMC_WIDTH_D1_ONLY
  ...
  regval = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_CLKENA);
  regval &= ~(SDMMC_CLKENA_ENABLE(priv->slot)
              | SDMMC_CLKENA_LOWPOWER(priv->slot));
  esp32s3_putreg(regval, BOSS1_ESP32S3_SDMMC_CLKENA);
  ...
  esp32s3_putreg(regval, BOSS1_ESP32S3_SDMMC_TMOUT);
}
...
static int esp32s3_attach(struct sdio_dev_s *dev)
{
  ...
  ret = esp32s3_setup_irq(this_cpu(), BOSS1_ESP32S3_PERIPH_SDIO_HOST,
                          1, BOSS1_ESP32S3_CPUINT_LEVEL);
  DEBUGASSERT(ret >= 0);

  /* Attach the SDIO interrupt handler */

  ret = irq_attach(BOSS1_ESP32S3_IRQ_SDIO_HOST, esp32s3_interrupt, dev);
  if (ret == OK)
    {
      ...
      esp32s3_putreg(0, BOSS1_ESP32S3_SDMMC_INTMASK);
      esp32s3_putreg(SDMMC_INT_ALL(priv->slot), BOSS1_ESP32S3_SDMMC_RINTSTS);

      /* Enable Interrupts to happen when the INTMASK is activated */

      regval  = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_CTRL);
      regval |= SDMMC_CTRL_INTENABLE;
      esp32s3_putreg(regval, BOSS1_ESP32S3_SDMMC_CTRL);
      ...
      up_enable_irq(BOSS1_ESP32S3_IRQ_SDIO_HOST);
    }
                               ...
                               unsigned int nblocks)
{
  ...
  esp32s3_putreg(blocklen, BOSS1_ESP32S3_SDMMC_BLKSIZ);
  esp32s3_putreg(blocklen * nblocks, BOSS1_ESP32S3_SDMMC_BYTCNT);
}
#endif
                             ...
                             size_t nbytes)
{
  ...
  esp32s3_putreg(SDMMC_FIFOTH_RXWMARK(1), BOSS1_ESP32S3_SDMMC_FIFOTH);

#ifdef CONFIG_BOSS1_ESP32S3_SDMMC_DMA
  /* Make sure that internal DMA is disabled */

  esp32s3_putreg(0, BOSS1_ESP32S3_SDMMC_BMOD);
  ...
  esp32s3_putreg(SDCARD_TRANSFER_ALL, BOSS1_ESP32S3_SDMMC_RINTSTS);
                             ...
                             size_t nbytes)
{
  ...
  esp32s3_putreg(SDMMC_FIFOTH_TXWMARK(BOSS1_ESP32S3_TXFIFO_DEPTH / 2),
                 BOSS1_ESP32S3_SDMMC_FIFOTH);
...
static int esp32s3_cancel(struct sdio_dev_s *dev)
{
  ...
  esp32s3_putreg(SDCARD_WAITALL_CLEAR, BOSS1_ESP32S3_SDMMC_RINTSTS);
...
static int esp32s3_waitresponse(struct sdio_dev_s *dev, uint32_t cmd)
{
  ...
  while ((esp32s3_getreg(BOSS1_ESP32S3_SDMMC_RINTSTS) & SDMMC_INT_CDONE) == 0)
    {
      if (clock_systime_ticks() - watchtime > timeout)
        {
          mcerr("ERROR: Timeout cmd: %04x STA: %08x RINTSTS: %08x\n",
                cmd, esp32s3_getreg(BOSS1_ESP32S3_SDMMC_STATUS),
                esp32s3_getreg(BOSS1_ESP32S3_SDMMC_RINTSTS));
  ...
  if (esp32s3_getreg(BOSS1_ESP32S3_SDMMC_RINTSTS) & SDCARD_INT_RESPERR)
    {
  ...
  esp32s3_putreg(SDCARD_CMDDONE_CLEAR, BOSS1_ESP32S3_SDMMC_RINTSTS);
  return ret;
}
                                ...
                                uint32_t *rshort)
{
  ...
  esp32s3_putreg(SDCARD_RESPDONE_CLEAR | SDCARD_CMDDONE_CLEAR,
                 BOSS1_ESP32S3_SDMMC_RINTSTS);
  *rshort = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_RESP0);
  mcinfo("CRC=%04x\n", *rshort);
                            ...
                            uint32_t rlong[4])
{
  ...
  if (rlong)
    {
      rlong[0] = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_RESP3);
      rlong[1] = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_RESP2);
      rlong[2] = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_RESP1);
      rlong[3] = esp32s3_getreg(BOSS1_ESP32S3_SDMMC_RESP0);
    }
                                ...
                                size_t buflen)
{
  ...
  esp32s3_putreg(regval, BOSS1_ESP32S3_SDMMC_BMOD);

  esp32s3_putreg(1, BOSS1_ESP32S3_SDMMC_PLDMND);
...
struct sdio_dev_s *sdio_initialize(int slotno)
{
  ...
  configure_pin(CONFIG_BOSS1_ESP32S3_SDMMC_CLK, priv->sdio_pins->clk, OUTPUT);
  configure_pin(CONFIG_BOSS1_ESP32S3_SDMMC_CMD, priv->sdio_pins->cmd,
                INPUT | OUTPUT | PULLUP);
  configure_pin(CONFIG_BOSS1_ESP32S3_SDMMC_D0, priv->sdio_pins->d0,
                INPUT | OUTPUT | PULLUP);
...
#endif /* CONFIG_BOSS1_ESP32S3_SDMMC */
