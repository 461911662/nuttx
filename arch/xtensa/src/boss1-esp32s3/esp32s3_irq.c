...
#define NO_CPUINT               BOSS1_ESP32S3_CPUINT_TIMER0

/* Priority range is 1-5 */

#define BOSS1_ESP32S3_MIN_PRIORITY    1
#define BOSS1_ESP32S3_MAX_PRIORITY    5
#define BOSS1_ESP32S3_PRIO_INDEX(p)   ((p) - BOSS1_ESP32S3_MIN_PRIORITY)

#ifdef CONFIG_BOSS1_ESP32S3_WIFI
#  define BOSS1_ESP32S3_WIFI_RESERVE_INT  (1 << BOSS1_ESP32S3_CPUINT_MAC)
#else
#  define BOSS1_ESP32S3_WIFI_RESERVE_INT  0
#endif
...
static uint8_t g_cpu0_intmap[BOSS1_ESP32S3_NCPUINTS];
#ifdef CONFIG_SMP
static uint8_t g_cpu1_intmap[BOSS1_ESP32S3_NCPUINTS];
#endif
...
static uint32_t g_cpu0_freeints = BOSS1_ESP32S3_CPUINT_PERIPHSET &
                                  ~BOSS1_ESP32S3_WIFI_RESERVE_INT;

#ifdef CONFIG_SMP
static uint32_t g_cpu1_freeints = BOSS1_ESP32S3_CPUINT_PERIPHSET;
#endif
...
{
  BOSS1_ESP32S3_INTPRI1_MASK,
  BOSS1_ESP32S3_INTPRI2_MASK,
  BOSS1_ESP32S3_INTPRI3_MASK,
  BOSS1_ESP32S3_INTPRI4_MASK,
  BOSS1_ESP32S3_INTPRI5_MASK
};

#ifdef CONFIG_BOSS1_ESP32S3_IRAM_ISR_DEBUG
...
static inline void xtensa_attach_fromcpu1_interrupt(void)
{
  ...
  cpuint = esp32s3_setup_irq(0, BOSS1_ESP32S3_PERIPH_INT_FROM_CPU1, 1,
                             BOSS1_ESP32S3_CPUINT_LEVEL);
  DEBUGASSERT(cpuint >= 0);

  /* Attach the inter-CPU interrupt. */

  irq_attach(BOSS1_ESP32S3_IRQ_INT_FROM_CPU1, (xcpt_t)esp32s3_fromcpu1_interrupt,
             NULL);

  /* Enable the inter-CPU interrupt. */

  up_enable_irq(BOSS1_ESP32S3_IRQ_INT_FROM_CPU1);
}
#endif
  ...
  if (intset != 0)
    {
      ...
      for (cpuint = 0, bitmask = 0xff;
           cpuint <= BOSS1_ESP32S3_CPUINT_MAX && (intset & bitmask) == 0;
           cpuint += 8, bitmask <<= 8);
      ...
      for (; cpuint <= BOSS1_ESP32S3_CPUINT_MAX; cpuint++)
        {
...
static int esp32s3_alloc_cpuint(int cpu, int priority, int type)
{
  ...
  DEBUGASSERT(priority >= BOSS1_ESP32S3_MIN_PRIORITY &&
              priority <= BOSS1_ESP32S3_MAX_PRIORITY);
  DEBUGASSERT(type == BOSS1_ESP32S3_CPUINT_LEVEL);

  /* Check if there are any level CPU interrupts available at the
   * requested interrupt priority.
   */

  mask = g_priority[BOSS1_ESP32S3_PRIO_INDEX(priority)] &
          BOSS1_ESP32S3_CPUINT_LEVELSET;
...
static void esp32s3_free_cpuint(int cpuint)
{
  ...
  DEBUGASSERT(cpuint >= 0 && cpuint <= BOSS1_ESP32S3_CPUINT_MAX);
...
void up_irqinitialize(void)
{
  ...
  g_irqmap[XTENSA_IRQ_TIMER0] = IRQ_MKMAP(0, BOSS1_ESP32S3_CPUINT_TIMER0);
  g_irqmap[XTENSA_IRQ_SWINT]  = IRQ_MKMAP(0, BOSS1_ESP32S3_CPUINT_SOFTWARE1);
  g_irqmap[XTENSA_IRQ_SWINT]  = IRQ_MKMAP(1, BOSS1_ESP32S3_CPUINT_SOFTWARE1);

#ifdef CONFIG_BOSS1_ESP32S3_WIFI
  g_irqmap[BOSS1_ESP32S3_IRQ_MAC] = IRQ_MKMAP(0, BOSS1_ESP32S3_CPUINT_MAC);
  g_irqmap[BOSS1_ESP32S3_IRQ_PWR] = IRQ_MKMAP(0, BOSS1_ESP32S3_CPUINT_PWR);
#endif
...
#ifdef CONFIG_BOSS1_ESP32S3_WIFI
  g_cpu0_intmap[BOSS1_ESP32S3_CPUINT_MAC] = CPUINT_ASSIGN(BOSS1_ESP32S3_IRQ_MAC);
  g_cpu0_intmap[BOSS1_ESP32S3_CPUINT_PWR] = CPUINT_ASSIGN(BOSS1_ESP32S3_IRQ_PWR);
  xtensa_enable_cpuint(&g_intenable[0], 1 << BOSS1_ESP32S3_CPUINT_MAC);
#endif
...
void up_disable_irq(int irq)
{
  ...
  if (irq < XTENSA_NIRQ_INTERNAL)
  ...
  else
    {
      ...
      int periph = BOSS1_ESP32S3_IRQ2PERIPH(irq);
      uintptr_t regaddr;
      uint8_t *intmap;

      DEBUGASSERT(periph >= 0 && periph < BOSS1_ESP32S3_NPERIPHERALS);
...
int esp32s3_cpuint_initialize(void)
{
  ...
  for (i = 0; i < BOSS1_ESP32S3_NPERIPHERALS; i++)
    {
#ifdef CONFIG_SMP
      if (cpu != 0)
        {
  ...
  memset(intmap, CPUINT_UNASSIGNED, BOSS1_ESP32S3_NCPUINTS);

  /* Special case the 6 internal interrupts.
   *
   *   CPU interrupt bit             IRQ number
   *   ----------------------------  ---------------------
   *   BOSS1_ESP32S3_CPUINT_TIMER0      6  XTENSA_IRQ_TIMER0  0
   *   BOSS1_ESP32S3_CPUINT_SOFTWARE0   7  Not yet defined
   *   BOSS1_ESP32S3_CPUINT_PROFILING  11  Not yet defined
   *   BOSS1_ESP32S3_CPUINT_TIMER1     15  XTENSA_IRQ_TIMER1  1
   *   BOSS1_ESP32S3_CPUINT_TIMER2     16  XTENSA_IRQ_TIMER2  2
   *   BOSS1_ESP32S3_CPUINT_SOFTWARE1  29  XTENSA_IRQ_SWINT   4
   */

  intmap[BOSS1_ESP32S3_CPUINT_TIMER0]    = CPUINT_ASSIGN(XTENSA_IRQ_TIMER0);
  intmap[BOSS1_ESP32S3_CPUINT_TIMER1]    = CPUINT_ASSIGN(XTENSA_IRQ_TIMER1);
  intmap[BOSS1_ESP32S3_CPUINT_TIMER2]    = CPUINT_ASSIGN(XTENSA_IRQ_TIMER2);
  intmap[BOSS1_ESP32S3_CPUINT_SOFTWARE1] = CPUINT_ASSIGN(XTENSA_IRQ_SWINT);
 ...
 *   flags    - An ORred mask of the BOSS1_ESP32S3_CPUINT_FLAG_* defines. These
...
int esp32s3_setup_irq(int cpu, int periphid, int priority, int flags)
{
  ...
  if ((flags & BOSS1_ESP32S3_CPUINT_EDGE) != 0)
    {
  ...
  cpuint = esp32s3_alloc_cpuint(cpu, priority, BOSS1_ESP32S3_CPUINT_LEVEL);
  if (cpuint < 0)
    {
  ...
  irq = BOSS1_ESP32S3_PERIPH2IRQ(periphid);

  DEBUGASSERT(periphid >= 0 && periphid < BOSS1_ESP32S3_NPERIPHERALS);
  DEBUGASSERT(cpuint >= 0 && cpuint <= BOSS1_ESP32S3_CPUINT_MAX);
  ...
  if ((flags & BOSS1_ESP32S3_CPUINT_FLAG_IRAM) != 0)
    {
...
uint32_t *xtensa_int_decode(uint32_t cpuints, uint32_t *regs)
{
  ...
  for (bit = 0, mask = 0xff;
       bit < BOSS1_ESP32S3_NCPUINTS && (cpuints & mask) == 0;
       bit += 8, mask <<= 8);

  /* Process each pending CPU interrupt */

  for (; bit < BOSS1_ESP32S3_NCPUINTS && cpuints != 0; bit++)
    {
      mask = 1 << bit;
      if ((cpuints & mask) != 0)
        {
