...
static void rtcio_dispatch(int irq, uint32_t *reg_status)
{
  ...
  for (i = 0; i < BOSS1_ESP32S3_NIRQ_RTCIO_PERIPH && status != 0; i++)
    {
...
static int rtcio_interrupt(int irq, void *context, void *arg)
{
  ...
  rtcio_dispatch(BOSS1_ESP32S3_FIRST_RTCIOIRQ_PERIPH, &last_status);
...
void esp32s3_rtcioirqinitialize(void)
{
  ...
  int cpu = this_cpu();
  g_rtcio_cpuint = esp32s3_setup_irq(cpu, BOSS1_ESP32S3_PERIPH_RTC_CORE,
                                     1, BOSS1_ESP32S3_CPUINT_LEVEL);
  DEBUGASSERT(g_rtcio_cpuint >= 0);

  /* Attach and enable the interrupt handler */

  DEBUGVERIFY(irq_attach(BOSS1_ESP32S3_IRQ_RTC_CORE, rtcio_interrupt, NULL));
  up_enable_irq(BOSS1_ESP32S3_IRQ_RTC_CORE);
}
#endif
...
void esp32s3_rtcioirqenable(int irq)
{
  ...
  DEBUGASSERT(irq >= BOSS1_ESP32S3_FIRST_RTCIOIRQ_PERIPH &&
              irq <= BOSS1_ESP32S3_LAST_RTCIOIRQ_PERIPH);

  /* Convert the IRQ number to the corresponding bit */

  bit = irq - BOSS1_ESP32S3_FIRST_RTCIOIRQ_PERIPH;

  /* Get the address of the GPIO PIN register for this pin */

  up_disable_irq(BOSS1_ESP32S3_IRQ_RTC_CORE);
